  ...
  if (cart.length === 0) {
    return (
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-center gap-4 px-4 py-24 text-center">
        ...
        <h1 className="font-heading text-2xl font-extrabold text-fg">Your bag is empty</h1>
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          {cart.map((item) => (
            <div
              key={item.id}
              className="flex gap-4 rounded-2xl border border-dark/10 bg-surface p-4"
            >
              <Link
              ...
              <div className="flex flex-1 flex-col justify-between">
                <div>
                  ...
                  <Link href={`/product/${item.product.id}`} className="text-sm font-bold text-fg hover:text-primary">
                    {item.product.name}
                  </Link>
                  <p className="mt-1 text-xs text-fg/50">Size: {item.size}</p>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-heading text-base font-extrabold text-fg">
              ...
              <button
                ...
                className="self-start rounded-full p-2 text-fg/30 hover:bg-primary/10 hover:text-primary"
        ...
        <div className="h-fit space-y-4 rounded-2xl border border-dark/10 bg-surface p-6">
          ...
          <div className="space-y-2 text-sm text-fg/60">
            ...
            {shipping > 0 && (
              <p className="text-xs text-fg/40">Add {formatPrice(999 - cartTotal)} more for free shipping</p>
            )}
          </div>
